...
# A repository may map extra git author names onto a seat that shepherds them
# (``REVIEW_AUTHOR_ALIASES="Some Author=fable"``).  The case that forced this:
# a model that is not a seat writes the code, and the commits land under the
# human's own git identity, so the seat gate above sees no seat and the whole
# loop silently does nothing.  An alias names who receives the wake; it never
# invents a new seat.
#
# A git author name is unauthenticated metadata that anyone can set, so an
# alias is honoured ONLY when the PR head lives in the base repository — a
# branch only collaborators can push.  Without that fence a stranger's fork PR
# against a public repo could spell the aliased name into a commit and help
# itself to the reviewer budget.  Real seat names carry the same fence for the
# same reason.
...
def author_aliases() -> Mapping[str, str]:
    ...
    Parsed from ``REVIEW_AUTHOR_ALIASES`` as ``Name=seat`` pairs separated by
    commas or newlines. An entry whose seat falls outside the actor grammar is
    refused here rather than discovered later as a dead-lettered wake, exactly
    as ``burn_actor`` refuses an unroutable burn seat.
    """
...
def seat_map(*, same_repo: bool) -> Mapping[str, str]:
    ...
    ``same_repo=False`` (a fork head) sees the built-in seats only: an alias is
    a repository-scoped grant, and a fork branch is writable by anyone.
    """
    if not same_repo:
        return SEAT_ACTORS
...
def head_is_same_repo(pull_request: Mapping[str, Any]) -> bool:
    ...
    Absence never reads as same-repo: a payload missing either side is treated
    as a fork, which costs at most one unrouted wake and never spends the
    reviewer budget on a branch a stranger controls.
    """
...
def choose_author(
    names: Iterable[str],
    fallback: str = "",
    seats: Mapping[str, str] = SEAT_ACTORS,
) -> str:
    ...
    first_burn = ""
    for name in names:
        if name in seats and seats[name] != burn:
            return name
        if name in seats and not first_burn:
            first_burn = name
...
def author_for_pr(
    ...
    Aliases are resolved lazily and only where they can change the answer: a
    repository with no ``REVIEW_AUTHOR_ALIASES``, or a PR a built-in seat
    already authored, makes exactly the API calls it made before aliases
    existed. The PR-detail fetch that carries the fork fence therefore lands
    only on the PRs an alias could newly claim.
    """
    names = [
        commit_author_name(item)
        for item in github.paginate(f"pulls/{pr_number}/commits")
    ]
    author = choose_author(names)
    if not author:
        ...
        actor = SEAT_ACTORS.get(head_author)
        if actor:
            return (head_author, actor)
        aliases = author_aliases()
        if not aliases:
            return None
        if not head_is_same_repo(github.get(f"pulls/{pr_number}")):
            return None
        candidate = choose_author([*names, head_author], seats=aliases)
        actor = aliases.get(candidate)
        return (candidate, actor) if actor else None
    return (author, SEAT_ACTORS[author])


def skill_only_pr(github: GitHubApi, pr_number: int) -> bool:
    """True when every file this PR touches sits under a skill corpus root.

    Renames contribute both names: a file moved out of (or into) the corpus
    makes the PR mixed, and a mixed PR rides the review loop like any other.
    An empty file list is not a skill PR — exemption is never inferred from
    absence.
    """
    paths: list[str] = []
    ...
    return bool(paths) and all(path.startswith(SKILL_PATH_PREFIXES) for path in paths)
...
def build_retrospective_messages(
    *,
    ...
    header = (
        ...
        "weave-doctrine; the composed boundary admits the merge, and "
        "Hákon's veto is retrospective.\n\n"
...
def explicit_clean_signal(
    ...
    """
    if codex_comment_kind(str(review.get("body") or "")) == CODEX_COMMENT_CLEAN:
        return True
...
def route_review(event: Mapping[str, Any] | None = None) -> None:
    ...
    if skill_only_pr(github, pr_number):
        print(
            f"skill-only PR {repository}#{pr_number} - exempt from the "
            "review loop (the weekly skill audit owns it); no wake"
        )
        return
...
def _scan_open_pull(
    ...
    *,
    ...
    pr_number = int(pull_request["number"])
    if skill_only_pr(github, pr_number):
        # No summon, no nudge, no redelivery, no exhaustion gate: the weekly
        # skill audit is the whole quality gate for a skill-only PR.
        print(f"skill-only PR {repository}#{pr_number} - review loop exempt")
        return (0, 0, 0)
    reviews = github.paginate(f"pulls/{pr_number}/reviews")
    head_sha = str(head["sha"])
    comments = github.paginate(f"issues/{pr_number}/comments")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("command", choices=("route", "nudge", "canary", "skill-audit"))
