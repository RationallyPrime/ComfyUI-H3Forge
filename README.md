...
In static plans (phase 0), interior starts are snapped to H3's five-latent cadence wherever the fixed anchors, minimum overlap and unchanged window count permit it. Active stagger phases retain their original geometry; re-snapping every phase can eliminate seam movement entirely. The exact tail may be off cadence; tight strides can also make interior snapping impossible (for example 25/8 at 427 latents). The context-plan receipt reports `cadence=5` and `off_cadence_starts`. Global target position IDs are still transplanted exactly, including at unsnapped starts; cadence alignment alone is not evidence of better dialogue.
